...
import {
  ...
  phosphorPhone,
  phosphorPhoneCallFill,
  phosphorPencilSimple,
  phosphorPaperclip,
  phosphorMicrophone,
  phosphorAt,
  phosphorSmiley,
  phosphorPaperPlaneTiltFill,
  phosphorDotsThreeBold,
  phosphorChecks,
} from '@ng-icons/phosphor'

type Tab = 'Activity' | 'Notes' | 'Deals' | 'Docs'
  ...
  template: `
    ...
    >
      ...
      >
        ...
        >
          <ng-icon name="phosphorArrowSquareOut" size="13" />
        </button>
        <button
          class="text-black/40 dark:text-white/40 hover:text-black dark:hover:text-white transition-colors"
        >
          <ng-icon name="phosphorXBold" size="13" />
        </button>
      </div>

      <!-- Identity -->
      <div class="px-4 pt-4 pb-3 flex items-start gap-3">
        <div class="w-14 h-14 rounded-full bg-gradient-to-br from-pink-300 to-rose-500 shrink-0"></div>
        <div class="flex flex-col gap-1 min-w-0 flex-1 mt-0.5">
          ...
          >
            <ng-icon name="phosphorBuildings" size="12" />
            Nordic Soft AB
          </span>
          <span
            class="inline-flex items-center gap-1.5 text-[12px] text-black/55 dark:text-white/55"
          >
            <ng-icon name="phosphorEnvelopeSimple" size="12" />
            emma&#64;nordicsoft.io
          </span>
          <span
            class="inline-flex items-center gap-1.5 text-[12px] text-black/55 dark:text-white/55"
          >
            <ng-icon name="phosphorPhone" size="12" />
            +1 (202) 555-0198
          </span>
        </div>
      </div>
      ...
      <div class="flex items-center gap-2 px-4 pb-3">
        ...
        >
          <ng-icon name="phosphorPhoneCallFill" size="13" />
          Schedule a Call
        </button>
        <button
        ...
        >
          <ng-icon name="phosphorPencilSimple" size="13" />
          Edit Profile
        </button>
      </div>
        ...
        @for (c of comments; track $index) {
          <div class="flex flex-col gap-1.5">
            <div class="flex items-center gap-2">
              <div
                class="w-6 h-6 rounded-full bg-gradient-to-br from-slate-300 to-slate-500 shrink-0"
              ></div>
              ...
              <ng-icon
                name="phosphorChecks"
                size="13"
                class="ml-auto text-black/30 dark:text-white/30"
              />
              <button
                class="text-black/30 dark:text-white/30 hover:text-black/70 dark:hover:text-white/70"
              >
                <ng-icon name="phosphorDotsThreeBold" size="14" />
              </button>
            </div>
            ...
            <div class="flex items-center gap-1.5 pl-8">
              ...
              >
                <ng-icon name="phosphorSmiley" size="11" />
              </button>
              @for (r of c.reactions; track $index) {
                <span
      ...
      >
        ...
        <div class="flex items-center gap-1 text-black/40 dark:text-white/40">
          ...
          >
            <ng-icon name="phosphorPaperclip" size="14" />
          </button>
          <button
            class="w-7 h-7 flex items-center justify-center rounded-md hover:bg-black/[0.04] dark:hover:bg-white/[0.06] hover:text-black/70 dark:hover:text-white/70 transition-colors"
          >
            <ng-icon name="phosphorMicrophone" size="14" />
          </button>
          <button
            class="w-7 h-7 flex items-center justify-center rounded-md hover:bg-black/[0.04] dark:hover:bg-white/[0.06] hover:text-black/70 dark:hover:text-white/70 transition-colors"
          >
            <ng-icon name="phosphorAt" size="14" />
          </button>
          <button
            class="w-7 h-7 flex items-center justify-center rounded-md hover:bg-black/[0.04] dark:hover:bg-white/[0.06] hover:text-black/70 dark:hover:text-white/70 transition-colors"
          >
            <ng-icon name="phosphorSmiley" size="14" />
          </button>
          <button
            class="ml-auto w-7 h-7 flex items-center justify-center rounded-md bg-[#f97316] text-white hover:bg-[#ea6a0c] transition-colors"
          >
            <ng-icon name="phosphorPaperPlaneTiltFill" size="13" />
          </button>
        </div>
      </div>
    </div>
...
export class ProfilePanelComponent {
  readonly tabs: Tab[] = ['Activity', 'Notes', 'Deals', 'Docs']
