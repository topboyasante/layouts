<script lang="ts">
  ...
  let selected = new Set<string>(['r4', 'r7', 'r8'])
...
<div class="flex-1 flex flex-col min-h-0 bg-white dark:bg-[#0f0f0f]">
  <!-- Tabs -->
  <div class="flex items-center gap-1 px-7 border-b border-black/[0.06] dark:border-white/[0.07] shrink-0">
    {#each tabs as tab (tab)}
      <button
        on:click={() => (activeTab = tab)}
        class="relative px-3 py-3 text-[13px] transition-colors {activeTab === tab
  ...
  <div class="flex items-center justify-between gap-3 px-7 py-3 shrink-0">
    <div class="flex items-center gap-2">
      <div class="flex items-center gap-2 px-3 h-9 w-[280px] rounded-lg bg-white dark:bg-white/[0.05] border border-black/[0.08] dark:border-white/[0.1]">
      ...
      <button class="w-9 h-9 inline-flex items-center justify-center rounded-lg border border-black/[0.08] dark:border-white/[0.1] text-black/55 dark:text-white/55 hover:bg-black/[0.03] dark:hover:bg-white/[0.05] hover:text-black dark:hover:text-white transition-colors">
    ...
    <div class="flex items-center gap-2">
      <div class="flex items-center gap-0.5 p-0.5 rounded-lg border border-black/[0.08] dark:border-white/[0.1]">
        <button class="w-7 h-7 flex items-center justify-center rounded-md bg-black/[0.05] dark:bg-white/[0.07] text-black/70 dark:text-white/70">
          <List size={14} />
        </button>
        <button class="w-7 h-7 flex items-center justify-center rounded-md text-black/40 dark:text-white/40 hover:bg-black/[0.04] dark:hover:bg-white/[0.05]">
          <SquaresFour size={14} />
        </button>
      </div>
      <button class="flex items-center gap-1.5 h-9 px-3 rounded-lg border border-black/[0.08] dark:border-white/[0.1] text-[13px] font-medium text-black/70 dark:text-white/70 hover:bg-black/[0.03] dark:hover:bg-white/[0.05] transition-colors">
  ...
  <div class="flex-1 overflow-auto px-7 pb-8">
    <table class="w-full text-[13px]">
      <thead>
        <tr class="text-black/40 dark:text-white/40 border-b border-black/[0.06] dark:border-white/[0.07]">
          <th class="w-9 py-3 text-left">
            <span class="inline-flex items-center justify-center w-4 h-4 rounded-[4px] border border-black/[0.15] dark:border-white/[0.18]" />
          </th>
      ...
      <tbody>
        {#each rows as r (r.id)}
          ...
          <tr
            class="border-b border-black/[0.04] dark:border-white/[0.05] last:border-0 transition-colors {isSelected
              ? 'bg-[#f97316]/[0.04]'
          ...
          >
            <td class="py-2.5">
              <button
                ...
                class="inline-flex items-center justify-center w-4 h-4 rounded-[4px] transition-colors {isSelected
            ...
            <td class="py-2.5 pr-4">
              <div class="flex items-center gap-2.5">
                <div class="w-7 h-7 rounded-full bg-gradient-to-br {r.avatar} shrink-0" />
                <span class="text-black dark:text-white whitespace-nowrap">{r.name}</span>
              </div>
            </td>
            <td class="py-2.5 pr-4 text-black/65 dark:text-white/65 whitespace-nowrap">{r.company}</td>
            <td class="py-2.5 pr-4 text-black/55 dark:text-white/55 whitespace-nowrap">{r.email}</td>
            <td class="py-2.5 pr-4 whitespace-nowrap">
              <span class="inline-flex items-center px-2 py-0.5 rounded-md border border-black/[0.1] dark:border-white/[0.12] text-[11px] font-medium tracking-wider text-black/55 dark:text-white/55">
                {r.status}
              </span>
            </td>
            <td class="py-2.5 pr-4 text-black/65 dark:text-white/65 whitespace-nowrap">{r.manager}</td>
            <td class="py-2.5 pr-4 text-black/55 dark:text-white/55 whitespace-nowrap">{r.source}</td>
            <td class="py-2.5 pr-4 whitespace-nowrap">
              <ScoreBar score={r.score} />
            </td>
            <td class="py-2.5 pr-4 text-black/55 dark:text-white/55 whitespace-nowrap">{r.date ?? ''}</td>
            <td class="py-2.5 text-right pr-1">
              <button class="w-7 h-7 inline-flex items-center justify-center rounded-md text-black/35 dark:text-white/35 hover:bg-black/[0.04] dark:hover:bg-white/[0.06] hover:text-black/70 dark:hover:text-white/70 transition-colors">
