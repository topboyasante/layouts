<script lang="ts">
  import { Plus } from '@phosphor-icons/svelte'
  ...
  let activeTab = $state<ChartTab>('1M')
</script>
      ...
      <div class="flex items-center gap-1">
        {#each chartTabs as tab (tab)}
          <button
            ...
            class="px-3 py-1 rounded-md text-xs font-medium transition-colors {activeTab === tab
    ...
    <div class="flex gap-4">
      ...
      <div class="flex-1 flex flex-col gap-1">
        <svg viewBox={`0 0 ${CHART_W} ${CHART_H}`} class="w-full h-40" preserveAspectRatio="none">
          ...
          <!-- Spend dots -->
          {#each spendPoints as [cx, cy], i (`sd-${i}`)}
            <circle {cx} {cy} r="3" fill="#6366f1" />
          {/each}

          <!-- Savings dots -->
          {#each savingsPoints as [cx, cy], i (`sv-${i}`)}
            <circle {cx} {cy} r="3" fill="#c8f53a" />
          {/each}
        </svg>
        ...
        <tbody>
          {#each transactions as t, i (i)}
            <tr class="border-b border-black/[0.04] dark:border-white/[0.04] last:border-0 hover:bg-black/[0.02] dark:hover:bg-white/[0.02] transition-colors">
              ...
              <td class="px-5 py-3">
                <span class="inline-flex items-center px-2 py-0.5 rounded-full text-[11px] font-medium {categoryColors[t.category] ?? 'bg-gray-100 text-gray-600 dark:bg-gray-800 dark:text-gray-300'}">
                  {t.category}
                </span>
              </td>
