<script lang="ts">
  import { Pencil } from '@phosphor-icons/svelte'
    ...
    <div class="flex-1 bg-white dark:bg-[#111] rounded-2xl p-5 border border-black/[0.06] dark:border-white/[0.06]">
      ...
      <div class="mt-2">
        <svg viewBox="0 0 520 190" class="w-full" style="height: 190px">
          <!-- Y-axis labels -->
          {#each yLabels as val, i (val)}
            <text x="18" y={30 + i * 28} text-anchor="middle" font-size="10" fill="#00000066" class="dark:fill-white/40">
              {val}
            </text>
          {/each}

          {#each bars as bar (bar.day)}
            {@const x = startX + bars.indexOf(bar) * (barWidth + barGap)}
              ...
              {#if bar.highlight}
                <g>
                  <rect
                    ...
                    filter="url(#shadow)"
                  />
                  <text x={x + 20} y={yBase - totalPx - 55} text-anchor="middle" font-size="9" fill="#00000066">Wednesday, 7 Jan 2025</text>
                  <circle cx={x - 18} cy={yBase - totalPx - 42} r={3} fill="#c5e840" />
                  <text x={x - 12} y={yBase - totalPx - 39} font-size="9" fill="#000000aa">Savings</text>
                  <text x={x + 70} y={yBase - totalPx - 39} text-anchor="end" font-size="9" fill="#000" font-weight="600">$240</text>
                  <circle cx={x - 18} cy={yBase - totalPx - 28} r={3} fill="#5a9e3a" />
                  <text x={x - 12} y={yBase - totalPx - 25} font-size="9" fill="#000000aa">Income</text>
                  <text x={x + 70} y={yBase - totalPx - 25} text-anchor="end" font-size="9" fill="#000" font-weight="600">$700</text>
                  <circle cx={x - 18} cy={yBase - totalPx - 14} r={3} fill="#f59e0b" />
                  <text x={x - 12} y={yBase - totalPx - 11} font-size="9" fill="#000000aa">Expenses</text>
                  <text x={x + 70} y={yBase - totalPx - 11} text-anchor="end" font-size="9" fill="#000" font-weight="600">$460</text>
                </g>
              {/if}
    ...
    <div class="flex-1 bg-white dark:bg-[#111] rounded-2xl p-5 border border-black/[0.06] dark:border-white/[0.06]">
      ...
      <div class="flex gap-0.5 mt-2 h-2 rounded-full overflow-hidden">
        {#each categories as cat (cat.label)}
          <div style="width: {cat.pct}%; background-color: {cat.color}"></div>
        {/each}
      </div>
