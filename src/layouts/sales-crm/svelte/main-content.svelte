<script lang="ts">
  import { CaretDown, SortAscending, DotsThree } from '@phosphor-icons/svelte'
  ...
  <div class="flex items-center justify-between">
    ...
    <div class="flex items-center gap-2">
      ...
      <button class="flex items-center gap-1.5 border border-black/10 dark:border-white/10 rounded-full px-3 py-1 text-[12px] text-black/55 dark:text-white/45">
        Sep 1 – Nov 30, 2023 <CaretDown size={10} />
      </button>
    </div>
  </div>
        ...
        {#each pipeline as s (s.label)}
          <div>
            ...
            <div class="h-1 bg-black/6 dark:bg-white/8 rounded-full">
              <div class="h-1 rounded-full" style="width: {s.pct}%; background-color: {s.color};"></div>
            </div>
          </div>
        {/each}
      </div>
      ...
      {#each platforms as p (p.name)}
        <div class="flex items-center gap-3">
          ...
          <div class="flex-1 bg-black/6 dark:bg-white/8 rounded-full h-1.5">
            <div class="h-1.5 rounded-full" style="width: {p.pct}%; background-color: {p.color};"></div>
          </div>
