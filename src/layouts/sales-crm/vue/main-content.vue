...
import { PhCaretDown, PhSortAscending, PhDotsThree } from '@phosphor-icons/vue'
  ...
  <div class="flex-1 min-w-0 overflow-y-auto bg-[#fafafa] dark:bg-[#0f0f0f] p-5 flex flex-col gap-4">
    ...
    <div class="flex items-center justify-between">
      ...
      <div class="flex items-center gap-2">
        ...
        <button class="flex items-center gap-1.5 border border-black/10 dark:border-white/10 rounded-full px-3 py-1 text-[12px] text-black/55 dark:text-white/45">
          Sep 1 – Nov 30, 2023 <PhCaretDown :size="10" />
        </button>
      </div>
    </div>
    ...
    <div class="grid grid-cols-3 gap-3">
      <div
        ...
        :key="c.label"
        :class="`bg-white dark:bg-[#111] rounded-xl p-4 border ${c.pink ? 'border-2 border-[#f03e6e]' : 'border-black/8 dark:border-white/8'}`"
      >
    ...
    <div class="grid grid-cols-[1fr_auto] gap-4">
      ...
      <div class="bg-white dark:bg-[#111] rounded-xl border border-black/8 dark:border-white/8 overflow-hidden">
        ...
        <table class="w-full">
          ...
          <tbody>
            <tr
              v-for="d in recentDeals"
              :key="d.name"
              class="border-b border-black/4 dark:border-white/4 last:border-0 hover:bg-black/[0.02] dark:hover:bg-white/[0.02] transition-colors"
            >
        ...
        <div class="flex flex-col gap-2.5">
          <div v-for="s in pipeline" :key="s.label">
            ...
            <div class="h-1 bg-black/6 dark:bg-white/8 rounded-full">
              <div class="h-1 rounded-full" :style="{ width: `${s.pct}%`, backgroundColor: s.color }" />
            </div>
          </div>
        </div>
      </div>
    ...
    <div class="bg-white dark:bg-[#111] rounded-xl p-5 border border-black/8 dark:border-white/8">
      <div class="flex items-center justify-between mb-4">
        ...
        <button class="flex items-center gap-1 text-[11px] text-black/35 dark:text-white/35 hover:text-black dark:hover:text-white">
          <PhSortAscending :size="13" /> Filters
        </button>
      </div>
      <div class="flex flex-col gap-4">
        <div v-for="p in platforms" :key="p.name" class="flex items-center gap-3">
          <div class="w-5 h-5 rounded-full shrink-0" :style="{ backgroundColor: p.color }" />
          ...
          <div class="flex-1 bg-black/6 dark:bg-white/8 rounded-full h-1.5">
            <div class="h-1.5 rounded-full" :style="{ width: `${p.pct}%`, backgroundColor: p.color }" />
          </div>
